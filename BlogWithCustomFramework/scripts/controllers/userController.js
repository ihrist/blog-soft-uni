class UserController{
    ...
    register(requestData){
        ...
        if(requestData.fullName.length < 5){
            showPopup('error', "Fullname must consist of at least 5 symbols.");
            return;
        }
        ...
        if(requestData.password.length < 8){
            showPopup('error', 'short passowrd');
            return;
        }
        ...
        function success(response){
            showPopup('success', 'You have registrated');
            redirectUrl("#/login");
        },
        function error(data){
